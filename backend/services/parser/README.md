...
`users`/`portfolio`/`securities_reader` (см. architecture-decisions.md,
...
## Статус разбора для приложения (2026-09-25)

`task_id` задачи = id записи `report_imports` в `portfolio` (её создаёт
gateway). Воркер:

- в начале ставит `processing` (`UpdateReportImportStatus`); `NotFound` —
  задача из времён до импортов, статус не ведётся; `FailedPrecondition` —
  повторная доставка уже завершённой задачи, обрабатывается как обычно
  (строки дедуплицируются);
- успех — `done` ставит сам `portfolio` в транзакции `ImportReport`
  (передаётся `report_import_id`); пустой отчёт — `done` с нулями;
- постоянные ошибки (брокер не поддерживается, файл не разобрался,
  `portfolio` отклонил отчёт) — `failed` с текстом для пользователя, для
  неизвестной бумаги — с её `SECID/BOARD`;
- временные ошибки (MinIO, `portfolio` недоступен) — requeue, статус
  остаётся `processing`.

Обновление статуса — best effort: исход задачи от него не зависит.
